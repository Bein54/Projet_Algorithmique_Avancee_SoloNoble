...
public class SoloNoble {
	...
	public boolean resoudreSoloNoble(int billes) {
		...
		// On stocke dans une variable la grille actuelle en copiant caractère par
		// caractère
		// Cela nous a permis de contourner le problème dû au stockage d'une grille
		// Si on sauvegardait toute la grille directement, on aurait un problème car la
		...
		if (billes == 1) {
			...
			// On continue tant que l'on a pas résolu et que l'on est pas arrivé à la fin de
			// la grille
				...
				while (!grilleValide && j < tablier.getNbColonnes()) {
					...
					if (tablier.getGrille()[i][j].equals(".")) {
						...
						while (!grilleValide && !tablier.getDeplacement().equals("fin")) {
							...
							// C'est un appel caractéristique de la récursivité avec retour arrière
	...
	public void ecrireToutesSolutions() {
		...
		for (int i = solutions.size(); i > 0; i--) {
			...
			// On affiche le numéro de l'étape en fonction du nombre d'éléments de la table
			// solutions
	...
	public static void main(String[] args) throws IOException {
		...
		// Si la résolution est impossible, cela veut dire que la grille ne pas valide
		// Et on l'indique à l'utilisateur
		if (!resolution)
			System.out.println("Grille impossible !");

		// On affiche le temps de recherche
		// En faisant la différence de nos deux variables et en divisant le résultat par
		// 1 000 000 000
